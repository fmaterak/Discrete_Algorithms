...
def johnson(data, nb_machines, nb_jobs):
    seq_current = u(data, nb_jobs) + v(data, nb_jobs) + [0]
    return seq_current, makespan(seq_current, data, nb_machines)[nb_machines - 1][nb_jobs]


# run Johnson
def run():
    machines, jobs, processing_time = read_file("test.txt")
